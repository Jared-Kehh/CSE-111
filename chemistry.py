def main():
    ...
    molar = compute_molar_mass(symbol, periodic_table)
    formula_name = get_formula_name(formula, molecules_dict)
    sum_proton = sum_protons(symbol, periodic_table)

    # Compute the number of moles in the sample.
    num_moles = mass/molar

    # Print the molar mass.
    print(round(molar, 5), "grams/mole")

    # Print the number of moles.
    print(round(num_moles, 5), "moles")
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for i in symbol_quantity_list:
        chem_list = i[num]
        chem_num = i[num + 1]
        chem_atomic_mass = periodic_table_dict[chem_list]
        chem_add = chem_atomic_mass[1] * chem_num
        chemical_list.append(chem_add)

    for _ in range(len(chemical_list)):
        molar_mass += chemical_list[_]
